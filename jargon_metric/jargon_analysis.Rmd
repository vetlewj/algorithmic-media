...
print(head(data))
print(str(data))
# count entries per category filtered by only the categories with a hundred entries or more
data_filtered = data[, .N, by = link_flair_text][N > 100]
```
...
data[, top_category := fifelse(link_flair_text %in% c("Biology", "Medicine", "Health", "Neuroscience",
                               ...
                               fifelse(link_flair_text %in% c("Chemistry", "Physics", "Earth Science", "Geology",
                                       ...
                                       fifelse(link_flair_text %in% c("Psychology", "Social Science", "Anthropology", "Economics"), "Social Sciences",
                                               fifelse(link_flair_text %in% c("Computer Science", "Engineering", "Materials Science", "Mathematics"), "Engineering and Technology",
...
stargazer(model_interaction_jargon_year, sanity_check,
          type = "text",
          omit = c("month", "jargon")
)
...
```
Let's see if the top categories are worse than the detailed categories
```{r}
stargazer(model_interaction_jargon_top_category, model_interaction_jargon_category, type = "text")
...
```{r}
data[, jargon_percentage_times_100 := (jargon_proportion + 0.00001) * 100]
model_log = lm(log(score + 0.01) ~ jargon_proportion:factor(top_category) +
  factor(year) +
  factor(month) +
  factor(top_category), data = data)
model_log_100_times = lm(log(score + 0.01) ~ jargon_percentage_times_100:factor(top_category) +
  factor(year) +
  factor(month) +
  factor(top_category), data = data)
model_log_log = lm(log(score + 0.01) ~ log(jargon_proportion + 0.00001):factor(top_category) +
  factor(year) +
  factor(month) +
  factor(top_category), data = data)
model_log_log_100_times = lm(log(score + 0.01) ~ log(jargon_percentage_times_100):factor(top_category) +
  factor(year) +
  factor(month) +
  factor(top_category), data = data)

stargazer(model_log, model_log_100_times, model_log_log, model_log_log_100_times, type = "text", omit = 1:17) # Why is this no differen?
```
```{r}
# summarizing this and making it nice
model_interaction_jargon_top_category_new = lm(score ~ jargon_proportion:factor(top_category) +
  factor(year) +
  factor(month) +
  factor(top_category), data = data)
stargazer(model_interaction_jargon_top_category_new, type = "text", omit = 1:17)
stargazer(model_log_100_times, model_log_log, type = "text", omit = 1:17)
