...
if __name__ == '__main__':
    ...
    parser.add_argument("--task", help="Task to run", required=True, choices=list(TASK_ID_MAP.keys()), type=str)
    parser.add_argument("--outfile", help="Location to save data", required=True, type=str)

    parser.add_argument("--seed", type=int, help="Random seed", default=1000)
    parser.add_argument("--model", help="Model ID or file location of model to use", default="roberta-base", type=str)
    ...
    paper_dataset, news_dataset, tweet_dataset, original_data = load_inference_dataset(args.data, task=args.task)
    pipeline = pipeline(
        model=args.model,
        tokenizer=tk,
        device=device,
    ...
    if args.task != 'generalization':
        # Paper scores
        paper_id_to_score = {}
        for item, out in tqdm(zip(paper_dataset, pipeline(KeyDataset(paper_dataset, "text"))), desc='Scoring papers',
                              total=len(paper_dataset)):
            paper_id_to_score[item['article_id']] = out['score'] if args.task == 'sensationalism' else int(
                out['label'].split('_')[1])

        news_id_to_score = {}
        for item, out in tqdm(zip(news_dataset, pipeline(KeyDataset(news_dataset, "text"))), desc='Scoring articles',
                              total=len(news_dataset)):
            news_id_to_score[item['article_id']] = out['score'] if args.task == 'sensationalism' else int(
                out['label'].split('_')[1])

        tweet_id_to_score = {}
        for item, out in tqdm(zip(tweet_dataset, pipeline(KeyDataset(tweet_dataset, "text"))), desc='Scoring tweets',
                              total=len(tweet_dataset)):
            tweet_id_to_score[item['article_id']] = out['score'] if args.task == 'sensationalism' else int(
                out['label'].split('_')[1])
                        ...
                        for j, sentence in enumerate(data['news'][news]['finding_sentences']):
                            for k, sentence in enumerate(data['full_paper_sentences']):
                                if f"{news}_{j}_{k}" in news_id_to_score:
                                    finding_sentence_scores.append(news_id_to_score[f"{news}_{j}_{k}"])
                                else:
                ...
                for j, tweet in enumerate(data['full_tweets']):
                    ...
                    for k, sentence in enumerate(data['full_paper_sentences']):
                        if f"{data['doi']}_{j}_{k}" in tweet_id_to_score:
                            finding_sentence_scores.append(tweet_id_to_score[f"{data['doi']}_{j}_{k}"])
                        else:
                            finding_sentence_scores.append(-1)
